 ...
 *       Filename:  cmd_hier.c
 *
 *    Description:  This file defines the structure for maintaining cmd
 *                  hierarchy
 *
 *        Version:  1.0
...
extern int ut_test_handler(param_t *param, ser_buff_t *tlv_buf,
...
param_t *libcli_get_mode_param() { return &mode_param; }

param_t *libcli_get_suboptions_param() { return &suboptions_param; }

param_t *libcli_get_cmd_expansion_param() { return &cmd_expansion_param; }

/* Function to be used to get access to above hooks*/

param_t *libcli_get_root(void) { return &root; }

param_t *libcli_get_do_hook(void) { return &do_hook; }

param_t *libcli_get_show_hook(void) { return &show; }

param_t *libcli_get_debug_hook(void) { return &debug; }

param_t *libcli_get_debug_show_hook(void) { return &debug_show; }

param_t *libcli_get_config_hook(void) { return &config; }

param_t *libcli_get_clear_hook(void) { return &clear; }

param_t *libcli_get_run_hook(void) { return &run; }

param_t *libcli_get_repeat_hook(void) { return &repeat; }

param_t *libcli_get_show_brief_extension_param(void) {
...
param_t *get_cmd_tree_cursor() { return cmd_tree_cursor; }

int is_user_in_cmd_mode() { return (get_cmd_tree_cursor() != &root); }
...
extern void place_console(char new_line);

extern int load_file_handler(param_t *param, ser_buff_t *b,
                             op_mode enable_or_disable);

void libcli_register_display_callback(
    param_t *param, display_possible_values_callback disp_callback) {
...
char *get_str_leaf_type(leaf_type_t leaf_type) {
...
show_ext_t get_show_extension_type(ser_buff_t *b) {
   ...
   if (strncmp(show_ext_tlv->leaf_id, SHOW_EXTENSION_PARAM,
               strlen(SHOW_EXTENSION_PARAM)))
      return none;
   if (strncmp(show_ext_tlv->value, SHOW_EXTENSION_PARAM_BRIEF,
               strlen(SHOW_EXTENSION_PARAM_BRIEF)) == 0)
      return brief;
   if (strncmp(show_ext_tlv->value, SHOW_EXTENSION_PARAM_DETAIL,
               strlen(SHOW_EXTENSION_PARAM_DETAIL)) == 0)
      return detail;
   if (strncmp(show_ext_tlv->value, SHOW_EXTENSION_PARAM_EXTENSIVE,
               strlen(SHOW_EXTENSION_PARAM_EXTENSIVE)) == 0)
      return extensive;
   assert(0);
   return none;
}
...
void init_libcli() {
   ...
   // init_serialized_buffer_of_defined_size(&file_read_buffer,
   // TLV_MAX_BUFFER_SIZE);
   ...
   init_param(&mode_param, CMD, MODE_CHARACTER, mode_enter_callback, 0, INVALID,
              0, "ENTER MODE");
   init_param(&suboptions_param, CMD, SUBOPTIONS_CHARACTER,
              display_sub_options_callback, 0, INVALID, 0, "Sub-Options");
   init_param(&cmd_expansion_param, CMD, CMD_EXPANSION_CHARACTER,
              display_cmd_expansion_callback, 0, INVALID, 0,
   ...
   static param_t help;
   init_param(&help, CMD, "help", show_help_handler, 0, INVALID, 0,
              "help how to use this CLI");
   libcli_register_param(&show, &help);
   set_param_cmd_code(&help, SHOW_HELP);

   /*show history*/
   static param_t show_history;
   init_param(&show_history, CMD, "history", show_history_callback, 0, INVALID,
              0, "Command history");
   libcli_register_param(&show, &show_history);
   set_param_cmd_code(&show_history, SHOW_HISTORY);

   static param_t no_of_commands;
   init_param(&no_of_commands, LEAF, "N", show_history_callback, 0, INT, "N",
              "No Of Commands to fetch");
   ...
   init_param(&show_resgistered, CMD, "registered", 0, 0, INVALID, 0,
              "registered");
   libcli_register_param(&show, &show_resgistered);

   static param_t show_resgistered_cmds;
   init_param(&show_resgistered_cmds, CMD, "commands",
              show_resgistered_cmd_handler, 0, INVALID, 0, "commands");
...

   {
      ...
      libcli_register_param(&debug, &ut);
      {
         ...
         init_param(&enable, LEAF, 0, ut_test_handler,
                    enable_disable_validation_handler, STRING, "ut-enable",
                    "enable | disable");
   ...
   init_param(&debug_show, CMD, "show", 0, 0, INVALID, 0,
              "debug show commands");
   libcli_register_param(&debug, &debug_show);

   static param_t debug_show_cmdtree;
   init_param(&debug_show_cmdtree, CMD, "cmdtree", show_cmd_tree, 0, INVALID, 0,
   ...
   /*configure hook*/
   init_param(&config, CMD, "config", config_mode_enter_handler, 0, INVALID, 0,
              "config cmds");
   ...
   init_param(&file_name, LEAF, 0, load_file_handler, 0, STRING, "file-name",
              "Name of the file");
   libcli_register_param(&load, &file_name);
   set_param_cmd_code(&file_name, CONFIG_LOAD_FILE);

   static param_t supportsave;
   init_param(&supportsave, CMD, "supportsave", 0, 0, INVALID, 0,
              "Collect Support Save Data");
   libcli_register_param(&config, &supportsave);

   static param_t supportsave_enable;
   init_param(&supportsave_enable, CMD, "enable", supportsave_handler, 0,
              INVALID, 0, "enable/disable Support Save Data Collection");
   ...
   /*  run terminate */
   {
      ...
      init_param(&terminate, CMD, "term", cli_terminate_handler, 0, INVALID, 0,
                 "Terminate appln");
   ...
   }
   {
      ...
      libcli_register_param(&run, &ut);
      {
         ...
         init_param(&ut_file_path, LEAF, 0, 0, 0, STRING, "ut-file-name",
                    "UT file name");
         libcli_register_param(&ut, &ut_file_path);
         {
            static param_t tc_no;
            init_param(&tc_no, LEAF, 0, ut_test_handler, 0, INT, "tc-no",
                       "Test Case Number");
   ...
   init_param(&do_hook, CMD, "DO_HOOK", 0, 0, INVALID, 0,
              "operational commands shortcut");
   do_hook.options[MODE_PARAM_INDEX] =
       libcli_get_suboptions_param(); // A hack, just fill it
   ...
   /*configure repeat*/
   init_param(&repeat, CMD, "repeat", repeat_last_command, 0, INVALID, 0,
              "repeat");
   ...
   init_param(&config_console_name_name, LEAF, 0, config_console_name_handler,
              0, STRING, "cons-name", "Name of Console");
   ...
   static param_t cls;
   init_param(&cls, CMD, CLEAR_SCR_STRING, clear_screen_handler, 0, INVALID, 0,
              "clear screen");
   HIDE_PARAM(&cls);
   libcli_register_param(0, &cls);

   static param_t exit_cmd;
   init_param(&exit_cmd, CMD, GOTO_ONE_LVL_UP_STRING, exit_cmd_handler, 0,
              INVALID, 0, "Move One Level Up");
   HIDE_PARAM(&exit_cmd);
   libcli_register_param(0, &exit_cmd);

   static param_t end_cmd;
   init_param(&end_cmd, CMD, GOTO_TOP_STRING, end_cmd_handler, 0, INVALID, 0,
              "Goto Top level");
   HIDE_PARAM(&end_cmd);
   libcli_register_param(0, &end_cmd);

   /*initialise show extension params*/
   init_param(&show_brief_extension, CMD, "brief", show_extension_param_handler,
              0, INVALID, 0, "brief output");
...
void init_param(
    param_t *param,          /* pointer to static param_t variable*/
   ...
   if (param_type == CMD) {
      ...
      strncpy(GET_CMD_NAME(param), cmd_name,
              MIN(CMD_NAME_SIZE, strlen(cmd_name)));
   ...
   strncpy(GET_PARAM_HELP_STRING(param), help,
           MIN(PARAM_HELP_STRING_SIZE, strlen(help)));
...
void support_cmd_negation(param_t *param) {
   ...
   param_t *negate_param =
       find_matching_param(&param->options[0], NEGATE_CHARACTER);
   ...
   init_param(no_param, NO_CMD, NEGATE_CHARACTER, negate_callback, 0, INVALID,
              0, "Command Negation");

   /*We cant leave the MODE_PARAM_INDEX empty,
    * so a hack - fill it with suboptions param. I dont see any implication of
    * this. We dont support MODE with negate cmd*/

   no_param->options[MODE_PARAM_INDEX] =
       libcli_get_suboptions_param(); // libcli_get_mode_param();
...
static void _dump_one_cmd(param_t *param, unsigned short tabs) {
...
void dump_cmd_tree() { _dump_one_cmd(&root, 0); }

extern void command_parser(void);

extern void enhanced_command_parser(void);

void start_shell(void) { command_parser(); }

/* Command Mode implementation */

param_t *get_current_branch_hook(param_t *current_param) {
...
void goto_top_of_cmd_tree(param_t *curr_cmd_tree_cursor) {
   ...
   do {
      ...
      memset(GET_LEAF_VALUE_PTR(curr_cmd_tree_cursor), 0,
...
void go_one_level_up_cmd_tree(param_t *curr_cmd_tree_cursor) {
   ...
   if (IS_PARAM_LEAF(curr_cmd_tree_cursor)) {
      memset(GET_LEAF_VALUE_PTR(curr_cmd_tree_cursor), 0,
             LEAF_VALUE_HOLDER_SIZE);
      serialize_buffer_skip(tlv_buff,
                            -1 * (int)sizeof(tlv_struct_t)); /*Rewind*/
...
void build_cmd_tree_leaves_data(
    ser_buff_t *tlv_buff, /*Output serialize buffer*/
