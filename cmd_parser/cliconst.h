 ...
 *       Filename:  cliconst.h
 *
 *    Description:  Constants used by libcli
 *
 *        Version:  1.0
...
#define DEFAULT_DEVICE_NAME "root@tcp-ip-stack"
...
typedef enum { brief, detail, extensive, none } show_ext_t;

/* Default Command Codes*/
/*CMD codes need not be unique, but should be unique with in same command
 * sharing the same command handler. For example, cmd code of SHOW_HISTORY and
 * SHOW_HISTORY_N should be distinct because both commands have the same cmd
 * handler */
