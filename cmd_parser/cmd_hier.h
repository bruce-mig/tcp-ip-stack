...
typedef int (*cmd_callback)(param_t *param, ser_buff_t *tlv_buf,
...
typedef enum { CMD, LEAF, NO_CMD } param_type_t;
...
#define GET_LEAF_TYPE_STR(param)                                               \
   (get_str_leaf_type(GET_PARAM_LEAF(param)->leaf_type))
...
#define IS_PARAM_MODE_ENABLE(param_ptr)                                        \
   (param_ptr->options[MODE_PARAM_INDEX] != NULL)
#define IS_PARAM_SUBOPTIONS_ENABLE(param_ptr)                                  \
   (param_ptr->options[SUBOPTIONS_INDEX] != NULL)

/*True if user is not operating in root level*/
int is_user_in_cmd_mode();

param_t *libcli_get_no_hook(void);

param_t *libcli_get_do_hook(void);

param_t *libcli_get_root(void);

param_t *libcli_get_mode_param();

param_t *libcli_get_suboptions_param();

param_t *libcli_get_cmd_expansion_param();

param_t *libcli_get_repeat_hook(void);

param_t *libcli_get_show_brief_extension_param(void);

static inline param_t **get_child_array_ptr(param_t *param) {
   return &param->options[0];
}

static inline int INVOKE_LEAF_USER_VALIDATION_CALLBACK(param_t *param,
                                                       char *leaf_value) {

   assert(param);
   assert(leaf_value);

   /*If validation fn is not registered, then validation is assumed to be
    * passed*/
...
#define IS_CURRENT_MODE_SHOW()                                                 \
   (get_current_branch_hook(get_cmd_tree_cursor()) == libcli_get_show_hook())
#define IS_CURRENT_MODE_DEBUG()                                                \
   (get_current_branch_hook(get_cmd_tree_cursor()) == libcli_get_debug_hook())
#define IS_CURRENT_MODE_CONFIG()                                               \
   (get_current_branch_hook(get_cmd_tree_cursor()) == libcli_get_config_hook())
#define IS_CURRENT_MODE_CLEAR()                                                \
   (get_current_branch_hook(get_cmd_tree_cursor()) == libcli_get_clear_hook())
...
static inline int is_cmd_string_match(param_t *param, const char *str,
                                      bool *ex_match) {
   ...
   *ex_match = false;
   ...
   int rc = (strncmp(param->cmd_type.cmd->cmd_name, str, str_len));
