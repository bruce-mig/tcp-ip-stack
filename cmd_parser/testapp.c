...
static void list_vlans(param_t *param, ser_buff_t *tlv_buf) {
...
int show_ip_igmp_groups_handler(param_t *param, ser_buff_t *tlv_buf,
...
int mtrace_handler(param_t *param, ser_buff_t *tlv_buf,
                   op_mode enable_or_disable) {
...
int config_router_name_handler(param_t *param, ser_buff_t *tlv_buf,
                               op_mode enable_or_disable) {
   set_device_name("router2");
   return 0;
}

int user_vlan_validation_callback(char *vlan_id) {
...
int main(int argc, char **argv) {
   ...
   init_param(&config_router, CMD, "router", 0, 0, INVALID, 0,
              "Configuration Router");
   libcli_register_param(config, &config_router);

   /*Level 2*/
   static param_t config_router_name;
   init_param(&config_router_name, CMD, "name", 0, 0, INVALID, 0,
   ...
   init_param(&igmp_config, CMD, "configuration", show_ip_igmp_groups_handler,
              0, INVALID, 0, "IGMP Protocol Configuration");
   ...
   init_param(&show_ip_igmp_groups, CMD, "groups", show_ip_igmp_groups_handler,
              0, INVALID, 0, "Ipv4 Group Address");
   ...
   init_param(&show_ip_igmp_groups_vlan_vlan, LEAF, 0,
              show_ip_igmp_groups_handler, user_vlan_validation_callback, INT,
              "vlan-id", "Vlan id(1-4095)");
   libcli_register_param(&show_ip_igmp_groups_vlan,
                         &show_ip_igmp_groups_vlan_vlan);

   /*Level 5*/
   static param_t show_ip_igmp_groups_group_ip;
   init_param(&show_ip_igmp_groups_group_ip, LEAF, 0,
              show_ip_igmp_groups_handler, 0, IPV4, "group-ip",
   ...
   init_param(&show_ip_igmp_groups_group_ip_vlan, CMD, "vlan", 0, 0, INVALID, 0,
              "Vlan");
   libcli_register_param(&show_ip_igmp_groups_group_ip,
                         &show_ip_igmp_groups_group_ip_vlan);

   /*level 7*/
   static param_t show_ip_igmp_groups_group_ip_vlan_vlan_id;
   init_param(&show_ip_igmp_groups_group_ip_vlan_vlan_id, LEAF, 0,
              show_ip_igmp_groups_handler, user_vlan_validation_callback, INT,
              "vlan-id", "Vlan id(1-4095)");
   libcli_register_param(&show_ip_igmp_groups_group_ip_vlan,
                         &show_ip_igmp_groups_group_ip_vlan_vlan_id);
   ...
   init_param(&source_ip, LEAF, 0, mtrace_handler, 0, IPV4, "source-ip",
              "Source Ipv4 address");
   ...
   init_param(&destination, CMD, "destination", 0, 0, INVALID, 0,
              "destination command");
   libcli_register_param(&source_ip, &destination);

   static param_t dest_ip;
   init_param(&dest_ip, LEAF, 0, mtrace_handler, 0, IPV4, "destination-ip",
   ...
   init_param(&group_ip, LEAF, 0, mtrace_handler, 0, IPV4, "group-ip",
   ...
   init_param(&group_ip2, LEAF, 0, mtrace_handler, 0, IPV4, "group-ip",
              "Multicast Group Ipv4 address");
