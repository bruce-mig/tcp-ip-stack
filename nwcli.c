 ...
 *       Filename:  nwcli.c
 *
 *    Description:  This file implements CLI commands to interact with the
 *                  project
 *
 *        Version:  1.0
 *        Created:  Monday 26 May 2025 20:12:17  CAT
 *       Revision:  1.0
 *       Compiler:  gcc
 *
...
int validate_vlan_id(char *vlan_value) {
...
static int show_nw_topology_handler(param_t *param, ser_buff_t *tlv_buf,
...
static int show_mac_handler(param_t *param, ser_buff_t *tlv_buf,
                            op_mode enable_or_disable) {
...
extern void layer3_ero_ping_fn(node_t *node, char *dst_ip_addr,
                               char *ero_ip_address);

static int ping_handler(param_t *param, ser_buff_t *tlv_buf,
                        op_mode enable_or_disable) {

   int CMDCODE;
   node_t *node;
   char *ip_addr = NULL, *ero_ip_addr = NULL;
   char *node_name;
   ...
   TLV_LOOP_BEGIN(tlv_buf, tlv) {
      ...
      if (strncmp(tlv->leaf_id, "node-name", strlen("node-name")) == 0)
         ...
         ip_addr = tlv->value;
      else if (strncmp(tlv->leaf_id, "ero-ip-address",
                       strlen("ero-ip-address")) == 0)
...
extern void delete_rt_table_entry(rt_table_t *rt_table, char *ip_addr,
                                  char mask);
extern void rt_table_add_route(rt_table_t *rt_table, char *dst, char mask,
                               char *gw, char *oif);

static int l3_config_handler(param_t *param, ser_buff_t *tlv_buf,
                             op_mode enable_or_disable) {
...
extern void interface_set_l2_mode(node_t *node, interface_t *interface,
                                  char *l2_mode_option);

extern void interface_unset_l2_mode(node_t *node, interface_t *interface,
                                    char *l2_mode_option);
extern void interface_set_vlan(node_t *node, interface_t *interface,
                               unsigned int vlan);
extern void interface_unset_vlan(node_t *node, interface_t *interface,
                                 unsigned int vlan);

static int intf_config_handler(param_t *param, ser_buff_t *tlv_buf,
                               op_mode enable_or_disable) {
...
void nw_init_cli() {
...

   {
      ...
      init_param(&topology, CMD, "topology", show_nw_topology_handler, 0,
                 INVALID, 0, "Dump Complete Network Topology");
...

      {
         ...
         libcli_register_display_callback(&node, display_graph_nodes);
         {
            ...
            init_param(&node_name, LEAF, 0, 0, validate_node_extistence, STRING,
                       "node-name", "Node Name");
            ...
            }
            {
               ...
               static param_t mac;
               init_param(&mac, CMD, "mac", show_mac_handler, 0, INVALID, 0,
                          "Dump Mac Table");
            ...
            }
            {
               ...
               static param_t rt;
               init_param(&rt, CMD, "rt", show_rt_handler, 0, INVALID, 0,
      ...
      libcli_register_display_callback(&node, display_graph_nodes);
      {
         ...
         libcli_register_param(&node, &node_name);
         {
            ...
            libcli_register_param(&node_name, &ping);
            {
               ...
               init_param(&ip_addr, LEAF, 0, ping_handler, 0, IPV4,
                          "ip-address", "Ipv4 Address");
               libcli_register_param(&ping, &ip_addr);
               set_param_cmd_code(&ip_addr, CMDCODE_PING);
               {
                  static param_t ero;
                  init_param(&ero, CMD, "ero", 0, 0, INVALID, 0,
                             "ERO(Explicit Route Object)");
                  libcli_register_param(&ip_addr, &ero);
                  {
                     static param_t ero_ip_addr;
                     init_param(&ero_ip_addr, LEAF, 0, ping_handler, 0, IPV4,
                                "ero-ip-address", "ERO Ipv4 Address");
         ...
         }
         {
            ...
            libcli_register_param(&node_name, &resolve_arp);
            {
               ...
               static param_t ip_addr;
               init_param(&ip_addr, LEAF, 0, arp_handler, 0, IPV4, "ip-address",
                          "Nbr IPv4 Address");
      ...
      libcli_register_display_callback(&node, display_graph_nodes);
      {
         ...
         libcli_register_param(&node, &node_name);
         {
            ...
            libcli_register_param(&node_name, &interface);
            {
               ...
               init_param(&if_name, LEAF, 0, 0, 0, STRING, "if-name",
                          "Interface Name");
               libcli_register_param(&interface, &if_name);
               {
                  /*config node <node-name> interface <if-name> l2mode*/
                  static param_t l2_mode;
                  init_param(&l2_mode, CMD, "l2mode", 0, 0, INVALID, 0,
                             "\"l2mode\" keyword");
                  libcli_register_param(&if_name, &l2_mode);
                  {
                     ...
                     init_param(&l2_mode_val, LEAF, 0, intf_config_handler,
                                validate_l2_mode_value, STRING, "l2-mode-val",
                                "access|trunk");
                     libcli_register_param(&l2_mode, &l2_mode_val);
                     set_param_cmd_code(&l2_mode_val,
                                        CMDCODE_INTF_CONFIG_L2_MODE);
                  }
               }
               {
                  /*config node <node-name> interface <if-name> vlan*/
                  static param_t vlan;
                  init_param(&vlan, CMD, "vlan", 0, 0, INVALID, 0,
                             "\"vlan\" keyword");
                  libcli_register_param(&if_name, &vlan);
                  {
                     ...
                     init_param(&vlan_id, LEAF, 0, intf_config_handler,
                                validate_vlan_id, INT, "vlan-id",
                                "vlan id(1-4096)");
...

         {
            ...
            libcli_register_param(&node_name, &route);
            {
               ...
               static param_t ip_addr;
               init_param(&ip_addr, LEAF, 0, 0, 0, IPV4, "ip-address",
                          "IPv4 Address");
               libcli_register_param(&route, &ip_addr);
               {
                  /*config node <node-name> route <ip-address> <mask>*/
                  static param_t mask;
                  init_param(&mask, LEAF, 0, l3_config_handler,
                             validate_mask_value, INT, "mask", "mask(0-32");
                  ...
                  set_param_cmd_code(&mask, CMDCODE_CONF_NODE_L3ROUTE);
                  {
                      ...
                      * <gw-ip>*/
                     static param_t gwip;
                     init_param(&gwip, LEAF, 0, 0, 0, IPV4, "gw-ip",
                                "IPv4 Address");
                     libcli_register_param(&mask, &gwip);
                     {
                         ...
                         * <gw-ip> <oif>*/
                        static param_t oif;
                        init_param(&oif, LEAF, 0, l3_config_handler, 0, STRING,
                                   "oif", "Out-going intf Name");
