...
extern void run_test_case(char *file_name, uint16_t tc_no);
...
char *get_last_command() { return last_command_input_buffer; }

param_t *find_matching_param(param_t **options, const char *cmd_name) {

   int i = 0, leaf_index = -1, j = 0, choice = -1;

   bool ex_match = false;

   memset(array_of_possibilities, 0,
          POSSIBILITY_ARRAY_SIZE * sizeof(param_t *));
   ...
   /*  Read \n and discard */
   if ((fgets((char *)cons_input_buffer, sizeof(cons_input_buffer) - 1,
              stdin) == NULL)) {
...
static CMD_PARSE_STATUS build_tlv_buffer(char **tokens, size_t token_cnt) {
   ...
   for (; i < token_cnt; i++) {
      ...
      if (param) {
         if (IS_PARAM_LEAF(param)) {
             ...
             * Below function performs basic standard sanity checks on the leaf
             * value input by the user */
            if (INVOKE_LEAF_LIB_VALIDATION_CALLBACK(param, *(tokens + i)) ==
                VALIDATION_SUCCESS) {

               /*Standard librray checks have passed, now call user validation
                * callback function*/
               if (INVOKE_LEAF_USER_VALIDATION_CALLBACK(param, *(tokens + i)) ==
                   VALIDATION_SUCCESS) {
               ...
               /*If leaf is not a valid value, terminate the command parsing
                * immediately*/
               status = INVALID_LEAF;
            }
            break;
   ...
   case CMD_NOT_FOUND:
      printf(ANSI_COLOR_RED
             "Error : Following Token not registered : %s\n" ANSI_COLOR_RESET,
             *(tokens + i));
      break;

   case INVALID_LEAF:
      printf(ANSI_COLOR_RED
             "Error : Following leaf value could not be validated"
   ...
   case COMPLETE:
      ...
      if (param == libcli_get_show_brief_extension_param()) {
         ...
         INVOKE_APPLICATION_CALLBACK_HANDLER(parent, tlv_buff,
                                             enable_or_disable);
      }
   ...
   case USER_INVALID_LEAF:
      printf(
          ANSI_COLOR_YELLOW
          "Error : User validation has failed : Invalid value for Leaf : %s\n",
          GET_LEAF_ID(param));
...
static void parser_process_repeat_cmd(char *next_token) {

   INVOKE_APPLICATION_CALLBACK_HANDLER(libcli_get_repeat_hook(), 0,
                                       OPERATIONAL);
}
...
parse_input_cmd(char *input, unsigned int len, bool *is_repeat_cmd) {
   ...
   if (token_cnt > 1 && ((strncmp(tokens[0], DO, 2) == 0)) &&
       (get_cmd_tree_cursor() !=
        libcli_get_root())) /*do commands are not allowed from root*/

   {
      if (IS_CURRENT_MODE_CONFIG()) /*Do commands are allowed only when user is
                                       operating in config mode*/
      {
         param_t *old_cursor_state = get_cmd_tree_cursor(),
                 *new_cursor_state = NULL;
         set_cmd_tree_cursor(
             libcli_get_do_hook()); /*It will also destroy the config mode
                                       serialize buffer*/

         /*There is a cross branch junp from config to Operational branch, hence
          * reset the buffer*/
         reset_serialize_buffer(tlv_buff);
         status = build_tlv_buffer(&tokens[1], token_cnt - 1);

         /*It might be possible that user may have switched the branch from
          * within config branch*/
         ...
         if (new_cursor_state != libcli_get_do_hook()) {
            ...
            build_cmd_tree_leaves_data(tlv_buff, libcli_get_root(),
                                       get_cmd_tree_cursor());
            mark_checkpoint_serialize_buffer(tlv_buff);
         }
      } else
   ...
   else if ((strncmp(tokens[0], GOTO_ONE_LVL_UP_STRING,
                     strlen(GOTO_ONE_LVL_UP_STRING)) == 0) &&
            (token_cnt == 1))
      go_one_level_up_cmd_tree(get_cmd_tree_cursor());

   else if ((strncmp(tokens[0], GOTO_TOP_STRING, strlen(GOTO_TOP_STRING)) ==
             0) &&
            (token_cnt == 1))
      goto_top_of_cmd_tree(get_cmd_tree_cursor());

   else if ((strncmp(tokens[0], CLEAR_SCR_STRING, strlen(CLEAR_SCR_STRING)) ==
             0) &&
            (token_cnt == 1))
      clear_screen_handler(0, 0, MODE_UNKNOWN);

   else if (!strncmp(tokens[0], "run", strlen("run")) &&
            !strncmp(tokens[1], "ut", strlen("ut")) && token_cnt == 4) {
      ...
      run_test_case(file_name, tc_no);
            ...
            !strncmp(tokens[1], "load", strlen("load")) && token_cnt == 3) {
...
void command_parser(void) {
   ...
   while (1) {
      ...
      if (status == COMPLETE && cmd_recording_enabled) {
         record_command(CMD_HIST_RECORD_FILE, cons_input_buffer,
      ...
      memcpy(last_command_input_buffer, cons_input_buffer,
             strlen(cons_input_buffer));
