...
void ut_parser_init() {
   ...
   if ((UT_PARSER_MSG_Q_FD = mq_open("/ut_parser_msg_q", O_RDWR | O_CREAT,
...
static void tc_append_result(glthread_t *head, uint16_t step_no, bool pass,
                             bool match) {
...
static void tc_print_result(glthread_t *head) {
...
static void tc_cleanup_result_list(glthread_t *result_head) {
...
bool run_test_case(char *file_name, uint16_t tc_no) {
   ...
   while ((fget_ptr = (fgets(line, sizeof(line), fp)))) {
      ...
      if (strncmp(line, ":TESTCASE-BEGIN:", strlen(":TESTCASE-BEGIN:")) == 0) {
         ...
         while (tc_no && current_tc_no != tc_no) {
            ...
            while ((fget_ptr = fgets(line, sizeof(line), fp))) {
               ...
               if (strncmp(line,
                           ":TESTCASE-BEGIN:", strlen(":TESTCASE-BEGIN:"))) {
                  continue;
               }
               break;
            }
         ...
         if (pattern_match(token, strlen(token), "show") &&
            ...
            if ((ut_parser_recv_buff_data_size = mq_timedreceive(
                     UT_PARSER_MSG_Q_FD, ut_parser_recv_buff, MAX_MSG_SIZE,
                     NULL, &mq_wait_time)) == -1) {

               printf("Msg Q  Time out : No Data Recvd from Backend\n");
               rc = sprintf(buff,
                            "Msg Q  Time out : No Data Recvd from Backend\n");
               fwrite(buff, 1, rc, ut_log_file);
               ...
               fwrite(buff, 1, rc, ut_log_file);
               fwrite(ut_parser_recv_buff, 1, ut_parser_recv_buff_data_size - 1,
                      ut_log_file);
               fwrite("\n", 1, 1, ut_log_file);
               ut_parser_recv_buff_data_size = 0;
               memset(ut_parser_recv_buff, 0, sizeof(ut_parser_recv_buff));
      ...
      else if (strncmp(line, ":PATTERN-MATCH:", strlen(":PATTERN-MATCH:")) ==
               0) {
         ...
         if (pattern_match(ut_parser_recv_buff, ut_parser_recv_buff_data_size,
                           pattern)) {
            printf("PASS\n");
      ...
      else if (strncmp(line, ":PATTERN-NOT-MATCH:",
                       strlen(":PATTERN-NOT-MATCH:")) == 0) {
         ...
         fwrite(buff, 1, rc, ut_log_file);
         fwrite(ut_parser_recv_buff, 1, ut_parser_recv_buff_data_size,
                ut_log_file);
         fflush(ut_log_file);
      }
         ...
         index = atoi(token);
         assert(index);
         int_store1 = string_fetch_integer(
             ut_parser_recv_buff, ut_parser_recv_buff_data_size, index);
         printf("int_store1 = %lu at index %d\n", int_store1, index);
         rc =
             sprintf(buff, "int_store1 = %lu at index %d\n", int_store1, index);
         fwrite(buff, 1, rc, ut_log_file);
         fflush(ut_log_file);
      }
         ...
         index = atoi(token);
         assert(index);
         int_store2 = string_fetch_integer(
             ut_parser_recv_buff, ut_parser_recv_buff_data_size, index);
         printf("int_store2 = %lu at index %d\n", int_store2, index);
         rc =
             sprintf(buff, "int_store2 = %lu at index %d\n", int_store2, index);
         fwrite(buff, 1, rc, ut_log_file);
         fflush(ut_log_file);
      }
         ...
         index = atoi(token);
         assert(index);
         int_store3 = string_fetch_integer(
             ut_parser_recv_buff, ut_parser_recv_buff_data_size, index);
         printf("int_store3 = %lu at index %d\n", int_store3, index);
         rc =
             sprintf(buff, "int_store3 = %lu at index %d\n", int_store3, index);
         fwrite(buff, 1, rc, ut_log_file);
         fflush(ut_log_file);
      }
   }
...
static void set_ut_debug_flag(char *ut_enable_flag) {
...
int ut_test_handler(param_t *param, ser_buff_t *tlv_buf,
