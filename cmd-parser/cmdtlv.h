...
#define EXTRACT_CMD_CODE(ser_buff_ptr)                                         \
   atoi(((tlv_struct_t *)(ser_buff_ptr->b) +                                   \
         (get_serialize_buffer_size(ser_buff_ptr) / sizeof(tlv_struct_t) - 1)) \
            ->value)
...
#define TLV_LOOP_BEGIN(ser_buff, tlvptr)                                       \
   {                                                                           \
      ...
      unsigned int i = 0, k = get_serialize_buffer_size(ser_buff) /            \
                              sizeof(tlv_struct_t);                            \
      for (; i < k - 1; i++, tlvptr++)
...
#define prepare_tlv_from_leaf(leaf, tlvptr)                                    \
   ...
   strncpy(tlvptr->leaf_id, leaf->leaf_id,                                     \
           MIN(LEAF_ID_SIZE, strlen(leaf->leaf_id)));

#define put_value_in_tlv(tlvptr, _val)                                         \
   strncpy(tlvptr->value, _val, MIN(LEAF_VALUE_HOLDER_SIZE, strlen(_val)));

static inline void print_tlv_content(tlv_struct_t *tlv) {
...
static inline bool parser_match_leaf_id(char *tlv_leaf_id,
                                        const char *leaf_id_manual) {

   int len;
   if ((len = strlen(tlv_leaf_id)) != strlen(leaf_id_manual))
      return false;
   return (strncmp(tlv_leaf_id, leaf_id_manual, len) == 0);
}

static inline void dump_tlv_serialized_buffer(ser_buff_t *tlv_ser_buff) {
...
static inline void swap_tlv_units(tlv_struct_t *tlv1, tlv_struct_t *tlv2) {
