...
int_validation_handler(leaf_t *leaf, char *value_passed) {
    ...
    *                             get_str_leaf_type(leaf->leaf_type),
    * value_passed);*/
...
int mode_enter_callback(param_t *param, ser_buff_t *b,
   ...
   if (param == libcli_get_root()) {
      printf(ANSI_COLOR_YELLOW
             "Info : Mode not supported at root level\n" ANSI_COLOR_RESET);
      return 0;
   }
...
int load_file_handler(param_t *param, ser_buff_t *b,
                      op_mode enable_or_disable) {
   ...
   TLV_LOOP_BEGIN(b, tlv) {
      ...
      if (strncmp(tlv->leaf_id, "file-name", strlen("file-name")) == 0) {
...
int supportsave_handler(param_t *param, ser_buff_t *b,
   ...
   case CONFIG_ENABLE:
      ...
      // signal(SIGSEGV, terminate_signal_handler);/*When process access illegal
      // memory*/ signal(SIGABRT, terminate_signal_handler);/*when process abort
      // itself*/
      break;
   case CONFIG_DISABLE:
...
int show_help_handler(param_t *param, ser_buff_t *b,
                      op_mode enable_or_disable) {
   ...
   printf("1. Use '%s' Character after the command to enter command mode\n",
          MODE_CHARACTER);
   printf("2. Use '%s' Character after the command to see possible follow up "
          "suboptions\n",
          SUBOPTIONS_CHARACTER);
   printf("3. Use '%s' from within the config branch to directly trigger "
          "operational commands\n",
          DO);
   printf("4. Use '%s' Character after the command to see possible complete "
          "command completions\n",
   ...
   printf("    d. config [%s] console name <console name> - set/unset new "
          "console name\n",
          NEGATE_CHARACTER);
   printf("    e. config [%s] supportsave enable - enable/disable supportsave "
          "facility\n",
          NEGATE_CHARACTER);
   ...
   printf(ANSI_COLOR_YELLOW "                      Author : Bruce Migeri, "
                            "Software Engineer\n" ANSI_COLOR_RESET);
   printf(ANSI_COLOR_YELLOW
          "                      Visit : github.com/bruce-mig for more "
          "projects\n" ANSI_COLOR_RESET);
   return 0;
}
...
int show_extension_param_handler(param_t *param, ser_buff_t *b,
   ...
   if (param == libcli_get_show_brief_extension_param()) {
      ...
      strncpy(tlv.leaf_id, SHOW_EXTENSION_PARAM,
