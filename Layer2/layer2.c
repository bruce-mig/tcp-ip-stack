...
void send_arp_broadcast_request(node_t *node, interface_t *oif, char *ip_addr) {
   ...
   ethernet_hdr_t *ethernet_hdr =
       (ethernet_hdr_t *)calloc(1, ETH_HDR_SIZE_EXCL_PAYLOAD + payload_size);

   if (!oif) {
      oif = node_get_matching_subnet_interface(node, ip_addr);
      if (!oif) {
         printf("Error : %s : No eligible subnet for ARP resolution for "
                "Ip-address : %s",
...
static void send_arp_reply_msg(ethernet_hdr_t *ethernet_hdr_in,
                               interface_t *oif) {

   arp_hdr_t *arp_hdr_in =
       (arp_hdr_t *)(GET_ETHERNET_HDR_PAYLOAD(ethernet_hdr_in));

   ethernet_hdr_t *ethernet_hdr_reply =
       (ethernet_hdr_t *)calloc(1, MAX_PACKET_BUFFER_SIZE);

   memcpy(ethernet_hdr_reply->dst_mac.mac, arp_hdr_in->src_mac.mac,
          sizeof(mac_add_t));
   ...
   arp_hdr_t *arp_hdr_reply =
       (arp_hdr_t *)(GET_ETHERNET_HDR_PAYLOAD(ethernet_hdr_reply));
   ...
   memcpy(arp_hdr_reply->dst_mac.mac, arp_hdr_in->src_mac.mac,
          sizeof(mac_add_t));
   ...
   char *shifted_pkt_buffer = pkt_buffer_shift_right(
       (char *)ethernet_hdr_reply, total_pkt_size, MAX_PACKET_BUFFER_SIZE);
...
static void process_arp_broadcast_request(node_t *node, interface_t *iif,
   ...
   if (strncmp(IF_IP(iif), ip_addr, 16)) {
      ...
      printf("%s : ARP Broadcast req msg dropped, Dst IP address %s did not "
             "match with interface ip : %s\n",
...
bool_t arp_table_entry_add(arp_table_t *arp_table, arp_entry_t *arp_entry,
    ...
    * do nothing*/
   if (arp_entry_old && IS_ARP_ENTRIES_EQUAL(arp_entry_old, arp_entry)) {
    ...
    * sane, then move the pending arp list from new to old one and return
    * FALSE*/
   if (arp_entry_old && arp_entry_sane(arp_entry_old) &&
...
static void pending_arp_processing_callback_function(
   ...
   memcpy(ethernet_hdr->dst_mac.mac, arp_entry->mac_addr.mac,
          sizeof(mac_add_t));
   memcpy(ethernet_hdr->src_mac.mac, IF_MAC(oif), sizeof(mac_add_t));
   SET_COMMON_ETH_FCS(
       ethernet_hdr, pkt_size - GET_ETH_HDR_SIZE_EXCL_PAYLOAD(ethernet_hdr), 0);
   send_pkt_out((char *)ethernet_hdr, pkt_size, oif);
}

static void process_arp_pending_entry(node_t *node, interface_t *oif,
                                      arp_entry_t *arp_entry,
                                      arp_pending_entry_t *arp_pending_entry) {
...
void arp_table_update_from_arp_reply(arp_table_t *arp_table, arp_hdr_t *arp_hdr,
                                     interface_t *iif) {
   ...
   if (arp_pending_list) {
      ...
      ITERATE_GLTHREAD_BEGIN(arp_pending_list, curr) {
         ...
         process_arp_pending_entry(iif->att_node, iif, arp_entry,
                                   arp_pending_entry);
...
void interface_set_l2_mode(node_t *node, interface_t *interface,
                           char *l2_mode_option) {
   ...
   /*case 4 : if interface is operating in access mode, and user config trunk
    * mode, then overwrite*/
   ...
   /* case 5 : if interface is operating in trunk mode, and user config access
    * mode, then overwrite, remove all vlans from interface, user must enable
    * vlan again on interface*/
...
void interface_unset_l2_mode(node_t *node, interface_t *interface,
                             char *l2_mode_option) {}

void interface_set_vlan(node_t *node, interface_t *interface,
...
void interface_unset_vlan(node_t *node, interface_t *interface,
                          unsigned int vlan) {}
...
void node_set_intf_vlan_membsership(node_t *node, char *intf_name,
                                    unsigned int vlan_id) {
...
extern bool_t is_layer3_local_delivery(node_t *node, uint32_t dst_ip);

static void l2_forward_ip_packet(node_t *node, unsigned int next_hop_ip,
   ...
   if (!arp_entry) {
      ...
      add_arp_pending_entry(arp_entry, pending_arp_processing_callback_function,
                            (char *)pkt, pkt_size);
...
static void layer2_pkt_receieve_from_top(node_t *node, unsigned int next_hop_ip,
                                         char *outgoing_intf, char *pkt,
                                         unsigned int pkt_size,
                                         int protocol_number) {
   ...
   if (protocol_number == ETH_IP) {
      ...
      l2_forward_ip_packet(node, next_hop_ip, outgoing_intf, empty_ethernet_hdr,
...
void demote_pkt_to_layer2(
    ...
    unsigned int
        next_hop_ip, /*If pkt is forwarded to next router, then this is Nexthop
                        IP address (gateway) provided by L3 layer. L2 need to
                        resolve ARP for this IP address*/
    char *outgoing_intf, /*The oif obtained from L3 lookup if L3 has decided to
                            forward the pkt. If NULL, then L2 will find the
                            appropriate interface*/
    char *pkt, unsigned int pkt_size, /*Higher Layers payload*/
    int protocol_number) { /*Higher Layer need to tell L2 what value need to be
                              feed in eth_hdr->type field*/

   layer2_pkt_receieve_from_top(node, next_hop_ip, outgoing_intf, pkt, pkt_size,
                                protocol_number);
}
...
void add_arp_pending_entry(arp_entry_t *arp_entry, arp_processing_fn cb,
                           char *pkt, unsigned int pkt_size) {
...
ethernet_hdr_t *tag_pkt_with_vlan_id(ethernet_hdr_t *ethernet_hdr,
                                     unsigned int total_pkt_size, int vlan_id,
   ...
   *new_pkt_size = 0;
   ...
   memcpy(vlan_ethernet_hdr->dst_mac.mac, ethernet_hdr_old.dst_mac.mac,
          sizeof(mac_add_t));
   memcpy(vlan_ethernet_hdr->src_mac.mac, ethernet_hdr_old.src_mac.mac,
          sizeof(mac_add_t));
...
ethernet_hdr_t *untag_pkt_with_vlan_id(ethernet_hdr_t *ethernet_hdr,
   ...
   *new_pkt_size = 0;
   ...
   ethernet_hdr =
       (ethernet_hdr_t *)((char *)ethernet_hdr + sizeof(vlan_8021q_hdr_t));

   memcpy(ethernet_hdr->dst_mac.mac, vlan_ethernet_hdr_old.dst_mac.mac,
          sizeof(mac_add_t));
   memcpy(ethernet_hdr->src_mac.mac, vlan_ethernet_hdr_old.src_mac.mac,
          sizeof(mac_add_t));
...
void layer2_frame_recv(node_t *node, interface_t *interface, char *pkt,
                       unsigned int pkt_size) {
   ...
   if (IS_INTF_L3_MODE(interface)) {
      ...
      if (vlan_id_to_tag) {
         pkt = (char *)tag_pkt_with_vlan_id((ethernet_hdr_t *)pkt, pkt_size,
                                            vlan_id_to_tag, &new_pkt_size);
