 ...
 *       Filename:  l2switch.c
 *
 *    Description:  This file defines routines and structues to implement L2
...
GLTHREAD_TO_STRUCT(mac_entry_glue_to_mac_entry, mac_table_entry_t,
                   mac_entry_glue);
...
#define IS_MAC_TABLE_ENTRY_EQUAL(mac_entry_1, mac_entry_2)                     \
   (strncmp(mac_entry_1->mac.mac, mac_entry_2->mac.mac, sizeof(mac_add_t)) ==  \
        0 &&                                                                   \
...
bool_t mac_table_entry_add(mac_table_t *mac_table,
   ...
   mac_table_entry_t *mac_table_entry_old =
       mac_table_lookup(mac_table, mac_table_entry->mac.mac);
...
static void l2_switch_perform_mac_learning(node_t *node, char *src_mac,
                                           char *if_name) {
...
static bool_t l2_switch_send_pkt_out(char *pkt, unsigned int pkt_size,
   ...
   case ACCESS: {
      ...
      if (vlan_8021q_hdr &&
         ...
         ethernet_hdr =
             untag_pkt_with_vlan_id(ethernet_hdr, pkt_size, &new_pkt_size);
...
void l2_switch_recv_frame(interface_t *interface, char *pkt,
                          unsigned int pkt_size) {
