 ...
 *       Filename:  layer2.h
 *
 *    Description: This file defines the structures required for Layer 2
 * functionality
 *
 *        Version:  1.0
...
typedef void (*arp_processing_fn)(node_t *, interface_t *oif, arp_entry_t *,
                                  arp_pending_entry_t *);
...
GLTHREAD_TO_STRUCT(arp_pending_list_to_arp_entry, arp_entry_t,
                   arp_pending_list);

#define IS_ARP_ENTRIES_EQUAL(arp_entry_1, arp_entry_2)                         \
   (strncmp(arp_entry_1->ip_addr.ip_addr, arp_entry_2->ip_addr.ip_addr, 16) == \
        0 &&                                                                   \
    strncmp(arp_entry_1->mac_addr.mac, arp_entry_2->mac_addr.mac, 6) == 0 &&   \
    strncmp(arp_entry_1->oif_name, arp_entry_2->oif_name, IF_NAME_SIZE) ==     \
        0 &&                                                                   \
...
void add_arp_pending_entry(arp_entry_t *arp_entry, arp_processing_fn, char *pkt,
...
#define VLAN_ETH_FCS(vlan_eth_hdr_ptr, payload_size)                           \
   (*(unsigned int *)((                                                        \
       (char *)(((vlan_ethernet_hdr_t *)vlan_eth_hdr_ptr)->payload) +          \
       payload_size)))
...
ALLOC_ETH_HDR_WITH_PAYLOAD(char *pkt, unsigned int pkt_size) {
   ...
   ethernet_hdr_t *eth_hdr =
       (ethernet_hdr_t *)(pkt - ETH_HDR_SIZE_EXCL_PAYLOAD);
...
l2_frame_recv_qualify_on_interface(interface_t *interface,
   ...
   *output_vlan_id = 0;
    ...
    * L2 mode provided that it is operational either in ACCESS mode or TRUNK
    * mode.*/
    ...
    * 2. Or  it must accept tagged frame but tagged with same vlan-id as
    * interface's vlan operation*/
   ...
   /* if interface is operating in a TRUNK mode, then it must discard all
    * untagged frames*/
   ...
   /*If the interface is operating in L3 mode, and recv vlan tagged frame, drop
    * it*/
