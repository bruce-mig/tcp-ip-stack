 ...
 *       Filename:  comm.c
 *
 *    Description:  This file contains the routines to implement the
...
int send_pkt_out(char *pkt, unsigned int pkt_size, interface_t *interface) {
   ...
   interface_t *other_interface =
       &interface->link->intf1 == interface ? &interface->link->intf2
...
extern void layer2_frame_recv(node_t *node, interface_t *interface, char *pkt,
                              unsigned int pkt_size);
...
int send_pkt_flood_l2_intf_only(node_t *node, interface_t *exempted_intf,
   ...
   for (; i < MAX_INTF_PER_NODE; i++) {
      ...
      if (intf == exempted_intf || !IF_L2_MODE(intf))
         continue;
