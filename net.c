 ...
 *
 *        Version:  1.0
 *        Created:  Friday 25 April 2025 01:28:17  CAT
 *       Revision:  1.0
 *       Compiler:  gcc
 *
...
extern void rt_table_add_direct_route(rt_table_t *rt_table, char *ip_addr,
                                      char mask);
...
void dump_intf_props(interface_t *interface) {
   ...
   if (interface->intf_nw_props.is_ipadd_config) {
      ...
      printf("\t MAC : %u:%u:%u:%u:%u:%u\n", IF_MAC(interface)[0],
             IF_MAC(interface)[1], IF_MAC(interface)[2], IF_MAC(interface)[3],
